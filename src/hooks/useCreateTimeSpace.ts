export const useCreateTimeSpace = () => {
  ...
  const createTimeSpace: (finishTime: string | undefined) => string = (
    ...
    const theFinishHour: number =
      finishMinute > 45 ? finishHour + 1 : finishHour;
    const theFinishMinute: number =
      finishMinute > 45 ? finishMinute - 60 : finishMinute;
  ...
  const adjustViewerTimeSpace: (finishTime: string) => string = (
    finishTime: string,
  ) => {
    ...
    // 数値調整
    const theFinishHour: number =
      finishMinute < 0 ? finishHour - 1 : finishHour;
    //const theFinishMinute: number = finishMinute < 0 ? 60 - parseInt(finishMinute.toString().split('-')[1]) : finishMinute;
    const theFinishMinute =
      finishMinute < 0 ? 60 - Math.abs(finishMinute) : finishMinute;
