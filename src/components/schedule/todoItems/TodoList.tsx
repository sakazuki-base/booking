...
function TodoList({ todoID }: { todoID: string }) {
  ...
  const OnViewModalWindow: (viewerParentElm: HTMLElement) => void = (
    viewerParentElm: HTMLElement,
  ) => {
    const modalWindow: Element | null = viewerParentElm.querySelector(
      `.${todoStyle.modalWindow}`,
    );
  ...
  const sortedTodoMemo: todoItemType[] = useMemo(() => {
    return [...todoMemo].sort((ahead, behind) => {
      if (
        typeof ahead.startTime !== "undefined" &&
        typeof behind.startTime !== "undefined"
      ) {
                  ...
                  {desktopView ? (
                    <div className={todoStyle.editTargetContent}>
                      {todoItem.todoContent.length > 6 ? (
                        <p className={todoStyle.editTargetStr}>
                          {todoItem.todoContent.slice(0, 6)}...
                        </p>
                      ) : (
                        <p className={todoStyle.editTargetStr}>
                      ...
                      {todoItem.startTime && todoItem.finishTime ? (
                        <span>
                          {todoItem.startTime} ～{" "}
                          {adjustViewerTimeSpace(todoItem.finishTime)}
                        </span>
                      ) : null}
                    </div>
                  ) : (
