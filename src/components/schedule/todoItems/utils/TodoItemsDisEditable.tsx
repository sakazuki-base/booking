...
function TodoItemsEditable({ props }: { props: TodoItemsEditableTypes }) {
  ...
  const handleCheckPassword: (e: ChangeEvent<HTMLInputElement>) => void = (
    e: ChangeEvent<HTMLInputElement>,
  ) => {
    // サニタイズした値をセッター関数（ setCheckPassword ）にセット
    const checkPasswordStr: string = handleInputValueSanitize(
  ...
  return (
    <div className={todoStyle.editFalseMode}>
      <div className={todoStyle.editTargetContent}>
        ...
        {todoItem.finishTime && (
          <p>終了時刻：{adjustViewerTimeSpace(todoItem.finishTime)}</p>
        )}
      </div>
      <label className={todoStyle.pwLabel}>
        <span>パスワード</span>
        <input
          type="text"
          value={checkPassword}
