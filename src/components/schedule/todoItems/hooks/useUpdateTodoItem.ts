...
export const useUpdateTodoItem = () => {
  ...
  /* ToDo（予約）の更新 */
  const updateTodoItem: (todoItems: todoItemType) => void = (
    ...
    const exceptRemoveTodoItems: todoItemType[] = [...todoMemo].filter(
      (todoItem) => todoItem.id !== updateTodoList.id,
    ); // 今回更新（削除）対象の todoItem 以外を返す
