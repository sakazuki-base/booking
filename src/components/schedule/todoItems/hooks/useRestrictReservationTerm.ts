...
export const useRestrictReservationTerm = () => {
  const restrictReservationTerm: (day: calendarItemType) => boolean = (
    ...
    const configReservableTerm_YearMonth: number = parseInt(
      `${ReservableTermYear}${ReservableTermMonth.toString().padStart(2, "0")}`,
    );
    const dayDate: number = parseInt(
      `${day.year}${day.month.toString().padStart(2, "0")}`,
    );

    const isValidationReservable: boolean =
      configReservableTerm_YearMonth < dayDate;
    if (isValidationReservable) {
      alert(
        `6ヶ月以内（${ReservableTermYear}/${ReservableTermMonth}まで）が予約可能（受け付け）期間です。`,
      );
      return true;
    }
