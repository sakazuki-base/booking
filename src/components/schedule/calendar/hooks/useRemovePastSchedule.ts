...
export const useRemovePastSchedule = () => {
  ...
  const removePastSchedule: (
  ...
  ) => void = (isMounted: boolean, fetchTodoMemo: todoItemType[]) => {
    if (isMounted && fetchTodoMemo.length > 0) {
      ...
      const exceptPastTodoMemos: todoItemType[] = [...fetchTodoMemo].filter(
        (memo) => {
          /* compareTarget_present と「同じ記述及び文字列型にする」ための整形処理 */
          const adjustMemoTimeData: string = memo.todoID
            .split("/")
            .map((d, i) => {
              if (i !== 0) {
