...
function TimeBlock({ props }: { props: TimeBlockType }) {
  ...
  const { useCreateTimeTableViewDay, getReservedInfo, checkLast15 } =
    useTimeBlock();

  const { hoverEventListener, leaveEventListener } = useCtrlToolTips();

  const theTimeTableViewDay: string =
    useCreateTimeTableViewDay(ctrlMultiTimeTable);

  // useMemo を使用した動的な予約情報（当日より1週間分の各部屋ごとのタイムテーブル配列）の取得
  const relevantReservations: todoItemType[] = useMemo(() => {
    return [...todoMemo].filter(
      (memo) =>
  ...
  return (
    <>
      {minBlocks.map((minBlock) => {
        const reservedInfo: reservedInfoType = getReservedInfo(
          relevantReservations,
          timeBlock,
          minBlock,
        );
        const isLast15: boolean = checkLast15(
          relevantReservations,
          timeBlock,
          minBlock,
        );
        ...
        return (
          <div
            ...
            data-info={`${
              reservedInfo.content.length > 16
                ? `${reservedInfo.content.slice(0, 16)}...`
                : reservedInfo.content
            }
                            ...
                            `}
            onMouseOver={
              reservedInfo.content.length > 0 ? hoverEventListener : undefined
            }
            onTouchStart={
              reservedInfo.content.length > 0 ? hoverEventListener : undefined
            }
