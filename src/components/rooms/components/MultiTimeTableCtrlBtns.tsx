...
function MultiTimeTableCtrlBtns({ props }: { props: ctrlBtnsProps }) {
  ...
  const ctrlNextTimeTable: (day: number) => void = (day: number) => {
    ...
    const isPassedThisMonth: boolean =
      day <= 7 && day >= oneWeekLater - thisLastDay;
  ...
  const handleCtrlMultiTimeTable: (
  ...
  return (
    <>
      <div className={roomStyle.multiTimeTableCtrlBtns}>
        <button onClick={handleCtrlMultiTimeTable} data-btn="prev">
          &lt; 前日
        </button>
        <button onClick={handleCtrlMultiTimeTable} data-btn="next">
          翌日 &gt;
        </button>
      </div>
      <ViewCurrentTimeTableDay
