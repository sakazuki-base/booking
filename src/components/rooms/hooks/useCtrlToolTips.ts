...
export const useCtrlToolTips = () => {
  ...
  // ツールチップの座標操作
  const _ctrlToolTips: (event: SyntheticEvent) => void = (
    ...
    const roomInfoToolTip: HTMLElement | null =
      document.querySelector(".roomInfoToolTip");
  ...
  const hoverEventListener: (e: SyntheticEvent) => void = (
    e: SyntheticEvent,
  ) => {
    const targetDataInfo: string | null =
