...
export const useTimeBlock = () => {
  ...
  const useCreateTimeTableViewDay: (ctrlMultiTimeTable: number) => string = (
  ...
  const getReservedInfo: (
    ...
    for (const todoItem of [...relevantReservations]) {
      const theTime = parseInt(
        `${timeBlock}${minBlock.toString().padStart(2, "0")}`,
      );
      ...
      if (theTime >= start && theTime <= finish) {
        reservedInfo = {
          ...
          // 終了時間はバッファ時間（15分）を減算
          finishTime:
            todoItem.finishTime && adjustViewerTimeSpace(todoItem.finishTime),
        };
      }
    }
  ...
  const checkLast15: (
    ...
    const bufferCheck: boolean[] = [...relevantReservations]
      ...
      .map((todoItem) => {
        ...
        const [finishHourStr, finishMinuteStr] =
          todoItem.finishTime!.split(":");
        ...
        const isInBuffer: boolean =
          targetTimeInMinutes >= bufferStartInMinutes &&
          targetTimeInMinutes < finishTimeInMinutes;
